      ...
      <Card size="small" title="媒体查询示例">
        <Space orientation="vertical" style={{ width: '100%' }} size="middle">
          ...
          <Tag color={isMobile ? 'success' : 'processing'}>
            useIsMobile：{isMobile ? '移动端' : '桌面端'}
          </Tag>
          <Tag color="default">样式见 doc/style.scss，按响应式容器宽度生效</Tag>
        </Space>
      </Card>
    </div>
  );
...
- 滚动加载
- 滚动到底部自动加载更多，移动端整行显示，PC 端卡片多列显示
- _ExampleDriver(@kne/current-lib_example-driver)[import * as _ExampleDriver from "@kne/example-driver"],(@kne/current-lib_example-driver/dist/index.css),antd(antd),remoteLoader(@kne/remote-loader)
...
const code = &#96;
const { Card, Spin, Tag, Space } = antd;
...
const makeItems = (start, count) =>
  Array.from({ length: count }, (_, i) => {
    ...
    return {
      id,
      title: '商品 ' + (id + 1),
      price: (id * 7 % 900 + 100).toFixed(2),
      tag: ['热销', '新品', '限时'][id % 3]
    };
  });

const Component = () => {
  // useIsMobile 由预览环境自动注入，按响应式容器宽度判断
  const isMobile = useIsMobile();
  const [items, setItems] = useState(() => makeItems(0, PAGE_SIZE));
  ...
  const loadMore = useCallback(() => {
    ...
    setTimeout(() => {
      setItems((prev) => [
        ...prev,
        ...makeItems(prev.length, Math.min(PAGE_SIZE, TOTAL - prev.length))
      ]);
      setLoading(false);
  ...
  return (
    <div style={{ padding: '12px' }}>
      <Space style={{ marginBottom: '12px' }} wrap>
        <Tag color={isMobile ? 'success' : 'processing'}>
          {isMobile ? '移动端：整行显示' : 'PC 端：卡片多列显示'}
        </Tag>
        <Tag color="default">已加载 {items.length} / {TOTAL}</Tag>
      </Space>
      <div
      ...
      >
        <div style={listStyle}>
          {items.map((item) => (
            <Card key={item.id} size="small" title={item.title}>
              <div style={{ color: '#f5222d', fontWeight: 600, marginBottom: '8px' }}>
                ¥ {item.price}
              </div>
        ...
        <div style={{ textAlign: 'center', padding: '16px 0', color: '#999' }}>
          {loading ? <Spin size="small" /> : hasMore ? '继续向下滚动加载更多' : '已经到底啦'}
        </div>
      </div>
    </div>
  );
...
render(<ExampleDriver list={[{
    title: '滚动加载',
    description: '滚动到底部自动加载更多；移动端整行显示，PC 端卡片多列自适应（切换设备预览查看差异）',
    code,
    scope
}]}/>);
